    ...
    public class ListViewCell : MonoBehaviour
    {
                ...
                if (isTitleCell)
                {
                    ...
                    if (this.titleBtn == null)
                    {
                        Debug.LogError(string.Format(
                            "gameObject [{1}],ListViewCell.isExpanding = true, but do not have a expand button!"));
                    }
                }

                return isTitleCell;
            }
            set { isTitleCell = value; }
        }
        ...
        public void SetCellInfo(CellInfo info, ListView listView = null)
        {
            ...
            if (this.cellInfo.IsTitle)
            {
                ...
                {
                    Debug.LogError(string.Format(
                        "gameObject [{0}],ListViewCell.isExpanding = true, but do not have a expand button!",
                        transform.name));
                }
            }
        }

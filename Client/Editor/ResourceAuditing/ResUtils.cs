...
namespace ResourceAuditing
{
    ...
    public static class ResUtils
    {
        ...
        public static void ColorLabelFieldTooltip(string title, string content, string tooltip, bool vaild,  int width = 0)
        {
            ...
            EditorGUILayout.LabelField(new GUIContent(title), new GUIContent(content, tooltip), vaild ? labelGreen : labelRed, width == 0 ? GUILayout.ExpandWidth(true) : GUILayout.Width(width));
        }
        #region 获取相同的文件
        /// <summary>
        /// 获取相同的文件

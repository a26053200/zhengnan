...
public class ByteReader
{
	...
	public BetterList<string> ReadCSV (bool newList=false,char splitchar = ',') //hrli
	{
		if (newList)
			mTemp = new BetterList<string>();
		else
			mTemp.Clear();
		string line = "";
	...
	//hrli add
	//public BetterList<string> ReadIdFirstCSV ()  //first field is Id,so place Id to last , then you can use Pop() get Id and reduce list count
	public BetterList<string> ReadIdFirstCSV (bool newList = false,char splitchar = ',') //hrli
	{
		if (newList)
			mTemp = new BetterList<string>();
		else
		...
		while (canRead)
		{
			...
			for (int i = wordStart, imax = line.Length; i < imax; ++i)
			{
				...
				if (ch == splitchar) //hrli
				{
					...
					{
						if (Id == null)
							Id = line.Substring(wordStart, i - wordStart);
						else
							mTemp.Add(line.Substring(wordStart, i - wordStart));
						wordStart = i + 1;
					}
				...
				else if (ch == '"')
				{
					if (insideQuotes)
					{
						...
						{
							if (Id == null)
								Id = line.Substring(wordStart, i - wordStart).Replace("\"\"", "\"");
							else
								mTemp.Add(line.Substring(wordStart, i - wordStart).Replace("\"\"", "\""));
							insideQuotes = false;
